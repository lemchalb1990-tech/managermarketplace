...
// Selección de empresa compartida por todo el módulo de Mercado Libre. Igual patrón que
// Facturación o Catálogo: el Super Admin elige la empresa antes de ver nada, y esa
// elección filtra preguntas, reclamos, devoluciones y calificaciones — nunca se mezclan
// entre empresas. El resto de los usuarios ya está acotado a la suya por el backend.
const STORAGE_KEY = 'mp_ml_company';

interface MlCompanyCtx {
...
const Ctx = createContext<MlCompanyCtx | null>(null);

export function useMlCompany(): MlCompanyCtx {
  const ctx = useContext(Ctx);
  if (!ctx) throw new Error('useMlCompany debe usarse dentro de <MlCompanyProvider>');
  return ctx;
}
      ...
      {selectedCompanyId && (
        <div className="flex flex-wrap items-center justify-between gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 mb-6">
          <p className="text-sm text-blue-900">
            Gestionando Mercado Libre de <strong>{selectedCompany?.name ?? 'la empresa seleccionada'}</strong>
          </p>
          <button
            onClick={openPicker}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-3">
              <p className="text-sm text-gray-500">
                Preguntas, reclamos, devoluciones y calificaciones de Mercado Libre se gestionan
                por empresa. Elige con cuál vas a trabajar.
              </p>
              <select
                autoFocus
