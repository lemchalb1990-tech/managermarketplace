...
export interface ProviderConfig {
  ...
  helpText?: string;
  helpUrl?: string;
  supportsDirectEmission?: boolean;
}
...
export default function ProviderPage({ config }: Props) {
  ...
  async function handleConnect() {
    ...
    try {
      const token = getToken()!;
      await api.billing.connections.create({
        provider: config.provider,
        name: formName.trim(),
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <a href="/dashboard/billing/invoices/new"
          ...
          {showForm && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
              <h3 className="font-semibold text-gray-800 mb-1">Conectar cuenta de {config.name}</h3>
              {config.helpText && <p className="text-xs text-gray-500 mb-1">{config.helpText}</p>}
              {config.helpUrl && (
                  ...
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right flex gap-3 justify-end">
                      ...
                      <button onClick={() => handleDelete(c.id, c.name)}
                        className="text-xs text-red-500 hover:text-red-700 font-medium">Desconectar</button>
                    </td>
                  </tr>
                ))}
                {connections.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-10 text-center text-gray-400">
                      ...
                      <p className="text-xs">Haz clic en "+ Conectar cuenta" para vincular tu cuenta de {config.name}.</p>
                    </td>
                  </tr>
                )}
              </tbody>
