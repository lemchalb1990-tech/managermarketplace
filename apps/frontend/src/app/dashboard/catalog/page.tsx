...
// Tarjeta de publicación por conexión de Ripley (Mirakl) — mismo patrón que
// ParisListingCard pero más simple: una sola categoría (sin family), sin homologación de
// atributos obligatorios (la API de Ripley no la expone) y descripción en texto plano (esta
// API no documenta soporte de HTML/fotos embebidas en la descripción, a diferencia de Paris).
function RipleyListingCard({ product, connection, currentUser, onRefresh }: {
  product: any; connection: { id: string; name: string }; currentUser: any; onRefresh: () => Promise<any>;
}) {
  const listing = product.listings?.find((l: any) => l.connectionId === connection.id);
  const channelPrice = product.channelPrices?.find((cp: any) => cp.connectionId === connection.id);
  ...
  const [title, setTitle] = useState(listing?.title ?? '');
  const [description, setDescription] = useState(listing?.description ?? '');
  const [categoryCode, setCategoryCode] = useState(savedAttrs?.categoryCode ?? '');
  const [categoryLabel, setCategoryLabel] = useState(savedAttrs?.categoryLabel ?? '');
  ...
  useEffect(() => {
    const token = getToken()!;
    api.connections.ripley.hierarchies(connection.id, token)
      // Solo categorías hoja (sin hijas) — Ripley exige elegir la más específica.
      .then((all) => {
        const parentCodes = new Set(all.map((c) => c.parentCode).filter(Boolean));
        setCategories(all.filter((c) => !parentCodes.has(c.code)));
      })
      .catch(() => {});
  }, [connection.id]);

  function selectCategoryByLabel(label: string) {
    const c = categories.find((x) => x.label === label);
    if (c) { setCategoryCode(c.code); setCategoryLabel(c.label); }
  }
  ...
  async function handleSaveDraft() {
    ...
    try {
      ...
      await api.connections.upsertListing(connection.id, product.id, {
        title: title.trim() || undefined,
        description,
        channelAttributes: categoryCode ? { categoryCode, categoryLabel, brand: brand.trim() || undefined } : undefined,
      }, token);
      await onRefresh();
    } catch (e: any) { setErr(e.message); }
    finally { setSaving(false); }
  ...
  async function handleDeleteListing() {
    if (!(await confirmDialog(
      '¿Eliminar esta sincronización con Ripley? Se borran el título, descripción, atributos y fotos propias de esta publicación (la publicación seguirá viva en Ripley, el sistema solo deja de rastrearla) — útil para volver a tomarla de cero.',
      { danger: true },
    ))) return;
    setDeletingListing(true);
    try {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="font-medium text-gray-900 text-sm">{connection.name}</p>
          <p className="text-xs text-gray-400">Ripley (Mirakl)</p>
        </div>
        {listing?.externalId ? (
          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Publicado</span>
        ) : (
      ...
      {listing?.externalId && (
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400">shop_sku:</span>
          <code className="text-xs font-mono bg-gray-50 border border-gray-200 rounded px-1.5 py-0.5 text-gray-600">{listing.externalId}</code>
        </div>
      )}
      {listing?.errorMsg && (
      ...
      {expanded && (
        <div className="space-y-3 border-t border-gray-100 pt-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Descripción en Ripley</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Categoría (Ripley) *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Precio de Venta Ripley</label>
            <div className="flex gap-2">
              <input type="number" step="0.01" min="0" value={priceInput} onChange={(e) => setPriceInput(e.target.value)}
                placeholder={`Igual al del catálogo ($${Number(product.price).toLocaleString('es-CL')}) si se deja vacío`}
                className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <label className="w-16 h-16 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center text-gray-400 text-xs cursor-pointer hover:border-pink-400 hover:text-pink-500">
                {uploadingImg ? '...' : '+ Foto'}
                <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={uploadingImg}
                  onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUploadImage(f); e.target.value = ''; }} />
              </label>
            ...
            {listingImages.length === 0 && (
              <p className="text-xs text-gray-400 mt-1">Sin fotos propias — se usarán las de la pestaña "Imágenes" al publicar (máx. 3).</p>
            )}
          </div>
      ...
      <div className="flex gap-2 flex-wrap pt-2 border-t border-gray-100">
        ...
        {listing?.externalId && (
          <button type="button" onClick={handleSync} disabled={syncing}
            title="Envía el stock y precio actuales a la publicación en Ripley"
            className="px-3 py-1.5 border border-gray-300 text-gray-600 rounded-lg text-xs font-medium hover:bg-gray-50 disabled:opacity-50">
            {syncing ? 'Sincronizando...' : 'Sincronizar'}
          </button>
        )}
        {listing && (currentUser?.role === 'SUPER_ADMIN' || currentUser?.role === 'COMPANY_ADMIN') && (
          <button type="button" onClick={handleDeleteListing} disabled={deletingListing}
            title="Borra el vínculo interno con Ripley (título, atributos, fotos propias) sin afectar la publicación real — para volver a tomarla de cero"
            className="px-3 py-1.5 border border-red-200 bg-red-50 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 disabled:opacity-50">
            {deletingListing ? 'Eliminando...' : 'Eliminar sincronización'}
          </button>
        )}
