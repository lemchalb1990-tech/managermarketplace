...
export class WalmartAdapter implements PlatformAdapter {
  ...
  private subTotal(order: any, type: string): number {
    const list = order.orderSummary?.orderSubTotals || [];
    const found = list.find((s: any) => s.subTotalType === type);
    return Number(found?.totalAmount?.amount ?? 0);
  }

  private commissionTotal(orderLines: any[]): number {
    let total = 0;
    for (const line of orderLines || []) {
      const charges = line.charges?.charge || [];
      const c = charges.find((c: any) => c.chargeType === 'COMMISSION');
      total += Number(c?.chargeAmount?.amount ?? 0);
    }
    return total;
  }
      ...
      try {
        ...
        await this.prisma.sale.create({
          data: {
            ...
            externalId: id,
            total: Number(o.orderSummary?.totalAmount?.amount ?? 0),
            shippingCost: this.subTotal(o, 'SHIPPING'),
            marketplaceFee: this.commissionTotal(orderLines),
            companyId,
